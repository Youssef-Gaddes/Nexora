...
public class UserDashboardController {
    ...
    @FXML
    private Label lblCurrencyConversions;
    @FXML
    ...
    private void loadUserWallet() {
        try {
            ...
            if (wallet != null) {
                ...
                lblCurrencyConversions.setText(exchangeRateService.formatAll(wallet.getBalance()));
            } else {
                lblWalletBalance.setText("0.00");
                lblWalletReserved.setText("0.00");
                lblCurrencyConversions.setText("€ 0.00 | $ 0.00 | £ 0.00");
            }
        } catch (Exception e) {
        ...
        if (result.isPresent() && result.get() > 0) {
            try {
                ...
                if (wallet != null) {
                    ...
                    transactionService.deposit(wallet.getWalletId(), amount, null);
                    ...
                    new Thread(() -> {
                        try {
                            ...
                            if (user != null && user.getEmail() != null) {
                                emailService.sendTransactionReceipt(user.getEmail(), "DEPOSIT", amount, walletId);
                            }
                        } catch (Exception ex) {
                            System.err.println("Email send failed (deposit): " + ex.getMessage());
                        }
                    }, "email-deposit").start();
        ...
        if (result.isPresent() && result.get() > 0) {
            try {
                ...
                if (wallet != null && wallet.getBalance().doubleValue() >= result.get()) {
                    BigDecimal amount = new BigDecimal(result.get());
                    transactionService.withdraw(wallet.getWalletId(), amount, null);
                    ...
                    new Thread(() -> {
                        try {
                            User user = userService.findById(userId);
                            if (user != null && user.getEmail() != null) {
                                emailService.sendTransactionReceipt(user.getEmail(), "WITHDRAW", amount, walletId);
                            }
                        } catch (Exception ex) {
                            System.err.println("Email send failed (withdraw): " + ex.getMessage());
                        }
                    }, "email-withdraw").start();
