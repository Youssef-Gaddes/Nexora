...
public class P2PContractViewController {
    ...
    @FXML
    private TableColumn<P2PContractRow, String> colListed;
    ...
    @FXML
    private TextField tfContractSearch;
    @FXML
    ...
    // Backing list for filter/sort
    private ObservableList<P2PContractRow> allContractRows = FXCollections.observableArrayList();
    ...
    public static class P2PContractRow {
        ...
        private final String listedAgo;
    ...
    private void setupTableColumns() {
        ...
        colListed.setCellValueFactory(
                cellData -> new javafx.beans.property.SimpleStringProperty(cellData.getValue().getListedAgo()));
    }
        ...
        try {
            ...
            for (P2PContract contract : openContracts) {
                if (contract.getCreatorId() != userId) { // Don't show own contracts
                    ...
                    String listedAgo = formatTimeAgo(contract.getCreatedAt());
            ...
            allContractRows.setAll(rows);
            contractsTable.setItems(allContractRows);
        } catch (Exception e) {
    ...
    public void filterContracts() {
        String query = tfContractSearch == null || tfContractSearch.getText() == null
                ? ""
                : tfContractSearch.getText().toLowerCase().trim();
        if (query.isEmpty()) {
            contractsTable.setItems(allContractRows);
        } else {
            ObservableList<P2PContractRow> filtered = FXCollections.observableArrayList();
            for (P2PContractRow row : allContractRows) {
                if (row.getCreatorName().toLowerCase().contains(query)
                        || row.getAssetName().toLowerCase().contains(query)) {
                    filtered.add(row);
                }
            }
            contractsTable.setItems(filtered);
        }
    }

    @FXML
    public void sortByReputation() {
        ObservableList<P2PContractRow> sorted = FXCollections.observableArrayList(allContractRows);
        sorted.sort((a, b) -> {
            double aScore = parseReputationScore(a.getReputationLabel());
            double bScore = parseReputationScore(b.getReputationLabel());
            return Double.compare(bScore, aScore); // descending
        });
        contractsTable.setItems(sorted);
    }

    /**
     * Parses the average rating out of a reputation label like "⭐ 4.5 | 12 ✔ | ..."
     */
    private double parseReputationScore(String label) {
        try {
            if (label == null || label.startsWith("Not"))
                return -1;
            String numPart = label.replace("⭐", "").trim().split("\\|")[0].trim();
    ...
    /**
     * Returns human-readable time since the contract was listed, e.g. "2 hours
     * ago".
     */
