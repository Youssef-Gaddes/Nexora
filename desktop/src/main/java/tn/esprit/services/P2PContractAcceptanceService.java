                    ...
                    try {
                        ...
                        if (asset != null) {
                            if (creator != null && creator.getEmail() != null) {
                                emailService.sendP2PTradeConfirmation(
                                        creator.getEmail(), creator.getFullName(), asset.getName(),
                                        contract.getQuantity(), contract.getTotalValue(), contractId);
                            }
                            if (acceptor != null && acceptor.getEmail() != null) {
                                emailService.sendP2PTradeConfirmation(
                                        acceptor.getEmail(), acceptor.getFullName(), asset.getName(),
                                        contract.getQuantity(), contract.getTotalValue(), contractId);
                            }
                        }
                    } catch (Exception ex) {
                        System.err.println("P2P Email failed: " + ex.getMessage());
                    }
                }, "email-p2p").start();
